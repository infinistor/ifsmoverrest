...
import com.pspace.ifsmover.rest.db.MariaDB;
...
public class Utils {
    ...
    public static DBManager getDBInstance() {
		if (dbManager == null) {
			if (RestConfig.getInstance().getDBRepository().equalsIgnoreCase(RestConfig.MARIADB)) {
				dbManager = MariaDB.getInstance();
			} else if (RestConfig.getInstance().getDBRepository().equalsIgnoreCase(RestConfig.SQLITEDB)) {
				dbManager = Sqlite.getInstance();
			} else {
